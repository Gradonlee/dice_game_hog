...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    '''last 2 digits called l2d'''
    l2d = opponent_score % 100
    tens = l2d // 10
    ones = l2d % 10
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    initial_strategy = strategy(0, 0)
    for score in range(goal):
        for opponent_score in range(goal):
            if initial_strategy != strategy(score, opponent_score):
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max = 0
    while i <= 10:
        curr = make_averaged(roll_dice, total_samples)(i, dice)
        if curr > max:
            max = curr
            num_rolls = i
        i += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    curr = tail_points(opponent_score)
    if curr > threshold:
        return 0
    else:
        if perfect_square(score+curr):
            return 0
        else:
            return num_rolls
